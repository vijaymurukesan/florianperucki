...
fn,false);else element.detachEvent("on"+ev,fn)},bindEventOnce:function(element,ev,fn){var me=this;var listener=function(){me.unbindEvent(element,ev,listener);fn.apply(this,arguments)};this.bindEvent(element,ev,listener)},dispatchEvent:function(element,type,properties){if(element){var event=document.createEvent("HTMLEvents",1,2);event.initEvent(type,true,true);Meny.extend(event,properties);element.dispatchEvent(event)}},getQuery:function(){var query={};location.search.replace(/[A-Z0-9]+?=([\w|:|\/\.]*)/gi,function(a){query[a.split("=").shift()]=a.split("=").pop()});return query}};if(typeof Date.now!=="function")Date.now=function(){return(new Date).getTime()};
var meny = Meny.create({menuElement: document.querySelector('#meny'),contentsElement: document.querySelector( '#contents' ),mouse: true,touch: true});